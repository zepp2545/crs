...
@section('title')
 {{isset($student) ? 'Edit Waiting Student' : 'Register Waiting Student'}}
@endsection
...
@section('content')
     ...
     <div class="card  mt-4 mx-auto">
       ...
       <div class="card-body card-default">
         <form action="{{isset($student) ? route('waitings.update',$student->id) : route('waitings.store')}}" method="post">
           @csrf
           @if(isset($student))
             @method('PUT')
           @endif
  ...
  $(document).ready(function(){
    ...
    $('#name_search').keypress(function(e){
      if(e.which==13){
        ...
        $.ajax({
          type:'POST',
          url:"{{route('waitings.get_stu_info')}}",
